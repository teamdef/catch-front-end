...
//
const imageTestApi = async (problems: ProblemTypes[], userId: number, setTitle: string): Promise<AxiosResponse> => {
  ...
  flatArr.forEach(async (imgBlob) => {
    const aaa = data.find((value: any) => value.fileName === imgBlob.name);
